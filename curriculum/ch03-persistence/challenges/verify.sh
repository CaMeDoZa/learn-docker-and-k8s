...
verify_ch02() {
    ...
    if curl -sf --connect-timeout 2 --max-time 5 http://127.0.0.1:3000 > /dev/null 2>&1; then
    ...
    if [ -n "$BIND_SOURCE" ] && [ -f "$BIND_SOURCE/index.js" ]; then
        ...
        ORIGINAL_RESPONSE=$(curl -sf --connect-timeout 2 --max-time 5 http://127.0.0.1:3000 2>/dev/null || echo "")
        ...
        NEW_RESPONSE=$(curl -sf --connect-timeout 2 --max-time 5 http://127.0.0.1:3000 2>/dev/null || echo "")
...
verify_ch03() {
    ...
    if [ "$LOG_EXISTS" = "yes" ]; then
        ...
        curl -sf --connect-timeout 2 --max-time 5 http://127.0.0.1:3000 > /dev/null 2>&1 || true
        sleep 1
        LOG_EXISTS=$(docker run --rm \
            -v learn-ch03-app-logs:/check \
            alpine sh -c "[ -f /check/requests.log ] && echo yes || echo no" 2>/dev/null || echo "no")
